...
template.add_rules(
    [
        ...
        *[(Relation.length(X, Y) <= Relation.length(X, Y, s)[1, ]) | meta for s in [Constant.short, Constant.long]],
        *[(Relation.sides(X, Y) <= Relation.sides(X, Y, s)[1, ]) | meta for s in [Constant.not_double, Constant.double]],
