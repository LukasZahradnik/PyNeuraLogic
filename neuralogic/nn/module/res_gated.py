...
class ResGatedGraphConv(Module):
    ...
    def __call__(self):
        ...
        gate = R.get(f"{self.output_name}__nij")

        metadata = Metadata(activation=Activation.IDENTITY, aggregation=self.aggregation)
        prod_metadata = Metadata(activation="product-identity", aggregation=self.aggregation)

        w = self.out_channels, self.in_channels

        return [
            (gate(V.I, V.J) <= (feature(V.I)[w], feature(V.J)[w])) | [Activation.SIGMOID],
            (head <= feature(V.I)[w]) | metadata,
